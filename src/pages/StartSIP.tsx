...
import { Check, ChevronRight, Info, Calendar, AlertCircle, PieChart, ArrowLeft } from 'lucide-react';
...
const StartSIP = () => {
  ...
  const [investmentType, setInvestmentType] = useState<'sip' | 'onetime'>('sip');
  ...
  const handleContinue = () => {
    if (step === 1) {
      // Validate amount
      const minAmount = investmentType === 'sip' ? 500 : 5000;
      if (!amount || parseInt(amount) < minAmount) {
        toast({
          title: "Invalid amount",
          description: `Minimum ${investmentType === 'sip' ? 'SIP' : 'one-time'} amount is ₹${minAmount.toLocaleString()}`,
          variant: "destructive",
        });
        return;
      }
      ...
      setTimeout(() => {
        ...
        toast({
          title: investmentType === 'sip' ? "SIP Started Successfully!" : "Investment Successful!",
          description: investmentType === 'sip' 
            ? `Your ${frequency.toLowerCase()} SIP of ₹${parseInt(amount).toLocaleString()} has been set up.`
            : `Your one-time investment of ₹${parseInt(amount).toLocaleString()} has been made.`,
        });
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white sticky top-0 z-10 shadow-sm">
        <div className="flex items-center p-4">
          ...
          <h1 className="text-lg font-medium line-clamp-1">Invest in {fund.name}</h1>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="bg-white rounded-lg shadow mb-4">
          <ToggleGroup 
            ...
            onValueChange={(value) => {
              if (value) setInvestmentType(value as 'sip' | 'onetime');
              setStep(1); // Reset step when changing investment type
            }}
            className="w-full border-b"
          >
            <ToggleGroupItem 
              value="sip" 
              className="w-1/2 data-[state=on]:bg-blue-50 data-[state=on]:text-fundeasy-blue h-12"
            >
              SIP
            </ToggleGroupItem>
            <ToggleGroupItem 
            ...
            <CardContent className="p-4 space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1 flex items-center">
                  <Info size={12} className="inline mr-1" /> Minimum {investmentType === 'sip' ? 'SIP' : 'one-time'} amount: 
                  ₹{investmentType === 'sip' ? '500' : '5,000'}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between py-2">
                  ...
                  <span className="font-medium">
                    {investmentType === 'sip' ? `SIP (${frequency})` : 'One-time'}
                  </span>
                </div>
                ...
                <div className="p-3 bg-blue-50 rounded-lg mt-4 flex">
                  ...
                  <p className="text-xs text-gray-700">
                    {investmentType === 'sip' ? (
                      ...
                      `By proceeding, you authorize FundEasy to debit ₹${parseInt(amount).toLocaleString()} 
                      from your bank account for this investment.`
                    )}
                  </p>
                </div>
              </div>
        ...
        <div className="mb-6">
          ...
          >
            {isProcessing ? 'Processing...' : step === 3 ? (
              investmentType === 'sip' ? 'Confirm & Start SIP' : 'Confirm & Invest'
            ) : 'Continue'}
